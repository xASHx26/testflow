...
class ExportEngine {
    ...
    const exportData = {
      ...
      steps: snapshot.steps.map(s => ({
        ...
        element: s.element ? {
          ...
          outerHTML: s.element.outerHTML ? s.element.outerHTML.substring(0, 3000) : null,
        } : null,
    ...
    switch (step.type) {
      ...
      case 'click': {
        if (useFallback) {
          ...
          L.push(`${indent}except ElementClickInterceptedException:`);
          L.push(`${indent}    driver.execute_script("arguments[0].click()", element)`);
        } else {
          const by = this._locatorToBy(best);
          L.push(`${indent}try:`);
          L.push(`${indent}    wait.until(EC.element_to_be_clickable((${by.by}, ${this._pyStr(by.value)}))).click()`);
          L.push(`${indent}except ElementClickInterceptedException:`);
          L.push(`${indent}    element = wait.until(EC.presence_of_element_located((${by.by}, ${this._pyStr(by.value)})))`);
          L.push(`${indent}    driver.execute_script("arguments[0].click()", element)`);
        }
        break;
      }
  ...
  _generateMarkdown(snapshot) {
    ...
    for (const step of snapshot.steps) {
      ...
      // Element HTML (outerHTML if available)
      if (step.element?.outerHTML) {
        L.push(`**Element HTML:**`);
        L.push(``);
        L.push('```html');
        const html = step.element.outerHTML.length > 1000
          ? step.element.outerHTML.substring(0, 1000) + '…'
          : step.element.outerHTML;
        L.push(html);
        L.push('```');
        L.push(``);
      }
    ...
    L.push(`---`);
    L.push(``);
    L.push(`*Report generated by TestFlow v1.0.0 — Test Automation IDE*  `);
    L.push(`*${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}*`);
    L.push(``);
